...
function makeCtx(overrides: Partial<AppContext> = {}): AppContext {
  return { appDef: { getDataTypes: () => [], getOptionSets: () => [], getPageNames: () => [], getPagePaths: () => [], getPageData: () => null } as any, mobileDef: null, client: null, editorClient: { loadPaths: async () => ({ last_change: 1, data: [] }) } as any, ...overrides };
}
...
describe('Structure Rules', () => {
  it('structure-empty-page: flags pages with zero elements', async () => {
    const ctx = makeCtx({
      appDef: { getDataTypes: () => [], getOptionSets: () => [], getPageNames: () => ['index', 'about'],
        getPagePaths: () => [{ name: 'index', id: 'a', path: '%p3.abc' }, { name: 'about', id: 'b', path: '%p3.def' }], getPageData: () => null } as any,
  ...
  it('structure-tiny-option-set: flags option sets with <2 options', () => {
    ...
    ], getPageNames: () => [], getPagePaths: () => [], getPageData: () => null } as any });
