...
function makeCtx(overrides: Partial<AppContext> = {}): AppContext {
  return { appDef: { getDataTypes: () => [], getOptionSets: () => [], getPageNames: () => [], getPagePaths: () => [] } as any, mobileDef: null, client: null, editorClient: { loadPaths: vi.fn().mockResolvedValue({ last_change: 1, data: [] }) } as any, ...overrides };
}
...
describe('Dead Code Rules', () => {
  ...
  it('dead-unused-type: does not flag User type', () => {
    const ctx = makeCtx({ appDef: { getDataTypes: () => [{ key: 'a', name: 'User', privacyRoles: {}, fields: {}, deepFields: [] }], getOptionSets: () => [], getPageNames: () => [], getPagePaths: () => [] } as any });
  ...
  it('dead-empty-field: flags fields with 0% population', async () => {
    ...
    ] }], getOptionSets: () => [], getPageNames: () => [], getPagePaths: () => [] } as any, client: mockClient as any });
  ...
  it('dead-empty-field: skips when no BubbleClient', async () => {
    const ctx = makeCtx({ appDef: { getDataTypes: () => [{ key: 'a', name: 'User', privacyRoles: {}, fields: {}, deepFields: [{ key: 'f1', name: 'bio', fieldType: 'text', isList: false, raw: {} }] }], getOptionSets: () => [], getPageNames: () => [], getPagePaths: () => [] } as any });
  ...
  it('dead-orphan-page: flags unreferenced pages', async () => {
    const ctx = makeCtx({ appDef: { getDataTypes: () => [], getOptionSets: () => [], getPageNames: () => ['index', 'orphan_page'],
      getPagePaths: () => [{ name: 'index', id: 'a', path: '%p3.abc' }, { name: 'orphan_page', id: 'b', path: '%p3.def' }] } as any,
